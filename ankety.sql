-- phpMyAdmin SQL Dump
-- version 5.0.2
-- https://www.phpmyadmin.net/
--
-- Počítač: 127.0.0.1
-- Vytvořeno: Pát 04. pro 2020, 13:20
-- Verze serveru: 10.4.13-MariaDB
-- Verze PHP: 7.4.7

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Databáze: `ankety`
--

-- --------------------------------------------------------

--
-- Struktura tabulky `odpovedi`
--

CREATE TABLE `odpovedi` (
  `id_odpovedi` int(11) NOT NULL,
  `id_otazky` int(11) NOT NULL,
  `text_odpovedi` varchar(1000) NOT NULL,
  `pocet_hlasu` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- --------------------------------------------------------

--
-- Struktura tabulky `otazky`
--

CREATE TABLE `otazky` (
  `id_otazky` int(11) NOT NULL,
  `text_otazky` varchar(1000) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

--
-- Klíče pro exportované tabulky
--

--
-- Klíče pro tabulku `odpovedi`
--
ALTER TABLE `odpovedi`
  ADD PRIMARY KEY (`id_odpovedi`);

--
-- Klíče pro tabulku `otazky`
--
ALTER TABLE `otazky`
  ADD PRIMARY KEY (`id_otazky`);

--
-- AUTO_INCREMENT pro tabulky
--

--
-- AUTO_INCREMENT pro tabulku `odpovedi`
--
ALTER TABLE `odpovedi`
  MODIFY `id_odpovedi` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=26;

--
-- AUTO_INCREMENT pro tabulku `otazky`
--
ALTER TABLE `otazky`
  MODIFY `id_otazky` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=20;
COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
